...
//		}				
...

	{
		...
		$data['pageTitle'] = $this->project_model->projectName().' :: products';
		...
		$config['base_url'] = base_url().'admin/products/p/';
		...
		$this->load->view('admin/products/productsView', $data);
	...
	public function add(){
		...
		$data['pageTitle'] = $this->project_model->projectName().' :: Products : Add New Product';
		$data['Message'] = '';	
		$data['heading'] = 'Products: Add New Product';
		$data['pageName'] = 'products';
		...
		$data['categoryDetail'] =  $this->general_model->getAllDataSimple('category', '100', '0', 'id', 'desc');
		
		if((bool)($this->input->post('btnAddproducts')) == TRUE){
			...
			if ($this->upload->do_upload()){
				...
				$Date = getCurrentDate();
				$Time = getCurrentTime();
				$DbFieldsAry = array('name', 'category', 'price', 'description', 'image', 'entry_date', 'status');
				$InfoAry = array($data['name'], $data['category'], $data['price'],$data['description'], $image_name, $Date, 'YES');				
				...
				if($this->allfunction->AddInfo_Simple($DbFieldsAry, $InfoAry, $this->Table)){
					...
					setMessage('success_message', 'Product is added successfully!');
					redirect(base_url().'admin/products', 'refresh');
				}
				else{
				...
				$this->load->view('admin/products/addproductsView', $data);
			}
		}
		else{
			$this->load->view('admin/products/addproductsView', $data);
	...
	public function delete($agentId = NULL, $page = NULL){		
		...
		if($agentId == NULL || $page == NULL){
			...
			setMessage('error_message', 'Unable to delete products, something went wrong from your side. Please try again later with proper procedure!');
		...
		if($this->general_model->getSingleValue($agentId, 'id', 'image', $this->Table)){
			...
			if($this->general_model->deleteData($agentId, 'id', $this->Table)){
				...
				redirect(base_url().'admin/products/p/'.$page, 'refresh');
	...
	private function checkPictureStatus($ID, $instituteID, $page, $picture){
		...
		if($QUERY->num_rows > 0){
			...
			return '<img src="'.base_url().'btPublic/admin/images/selected.png" style="margin:5px;" title="This is set as cover image!">
					<a href="'.base_url().'admin/products/removePicture/'.$ID.'/'.$instituteID.'/'.$page.'/'.urlencode($picture).'/YES"><img src="'.base_url().'btPublic/admin/images/delete.png" style="margin:5px;"></a>
					';
		}
		
		return '<a href="'.base_url().'admin/products/changePictureStatus/'.$instituteID.'/'.$ID.'/'.$page.'" title="Click to set as cover image?"><img src="'.base_url().'btPublic/admin/images/not_selected.png" style="margin:5px;" title="Click to set as cover image?"></a>
				<a href="'.base_url().'admin/products/removePicture/'.$ID.'/'.$instituteID.'/'.$page.'/'.urlencode($picture).'/NO"><img src="'.base_url().'btPublic/admin/images/delete.png" style="margin:5px;"></a>
				';
	}
	...
	public function changePictureStatus($instituteId = 0, $pictureId = 0, $page = 0){
		...
		redirect(base_url().'admin/products/update/'.$instituteId.'/'.$page, 'refresh');
	}
	...
	private function deleteInstitueImage($instituteID){
		...
		if($QUERY->num_rows >0){
			...
			foreach($QUERY->result() as $row){	
				$picture = $row->image;
				if($this->general_model->deleteData($row->id, 'id', 'product')){
	...
	public function update($ID = NULL, $page = NULL){				
		...
		$data['pageTitle'] = $this->project_model->projectName().' :: Products : Update Product';
		$data['Message'] = '';	
		$data['heading'] = 'Products: Update Product';
		$data['pageName'] = 'products';
		...
		if((bool)($this->input->post('btnUpdateproducts')) == TRUE){
			...
			if($this->form_validation->run() === TRUE){
				...
				if($isValid){
				$DbFieldsAry = array('name', 'category', 'price', 'description', 'image');
				$InfoAry = array($data['name'], $data['category'], $data['price'], $data['description'], $image_name);
											
				$this->general_model->updateInfo_Simple($ID, 'id', $DbFieldsAry, $InfoAry, $this->Table);				
											
				setMessage('success_message', 'Product updated successfully!');
				redirect(base_url().'admin/products/p/'.$page, 'refresh');
				}
				else{
				$this->load->view('admin/products/editproductsView', $data);
				}
			}
			else{
				$this->load->view('admin/products/editproductsView', $data);
	...
	private function deleteAll($agentId){		
		...
		if($this->general_model->deleteData($agentId, 'id', $this->Table)){
			...
			//$this->general_model->addAgentActivity($this->session->userdata('agentId'), $this->Table, $agentId, 'removed products', getCurrentDate(), getCurrentTime());										
	...
	public function addPicture($ID = 0, $page = 0){
		...
		if ($this->upload->do_upload()){
			...
			redirect(base_url().'admin/products/update/'.$ID.'/'.$page,'refresh');
		}
		else{
			$error = array($this->upload->display_errors());
	...
	public function removePicture($ID = 0, $institueId, $page = 0, $picture, $default){
		...
		if($this->general_model->deleteData($ID, 'ID', 'tbl_institute_pictures')){
			...
			redirect(base_url().'admin/products/update/'.$institueId.'/'.$page,'refresh');
