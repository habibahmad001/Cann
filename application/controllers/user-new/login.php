...
class Login extends CI_Controller {
...

	{
		...
		if($this->isUserLoggedin()){
			...
			redirect(base_url().'user/dashboard', 'refersh');
	...
	private function validateUserInfo($userName, $password){
		...
		if((bool)($dbInfo) == TRUE){
			...
			if($dbInfo->password == $password && $userName == $dbInfo->user_name){
				$sessData = array('userId' => $dbInfo->id,'User_Name' => $userName, 'password' => $password, 'REMOTE_ADDR' => $REMOTE_ADDR, 'FlagType' => $dbInfo->flag, 'recordsPerPage' => 20);
