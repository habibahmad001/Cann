...
<body>
<form action="<?php if(isset($ID)) $ID = $ID; else $ID = ""; echo site_url("/user/" . $pageName . "/update/" . $ID . "/" . $qid);?>" method="post" enctype="multipart/form-data" name="frm">
<div id="wrapper">
<input name="pagename" id="pagename" type="hidden" value="<?php echo site_url("/user/" . $pageName);?>" />
<input name="catid" id="catid" type="hidden" value="<?php echo $qid;?>" />
<table width="60%" border="0" class="edit_course" style="float: right; border: 1px solid rgb(204, 204, 204); margin: 2% 19% 0px 0px; <?php if(!strpos($_SERVER['REQUEST_URI'], "/Edit/")) echo "display:none;";?>">
  ...
  <tr>
    <td>&nbsp;</td>
    <td colspan="2" align="center"><h1>Edit Question</h1></td>
    <td style="text-align:right;" align="right" valign="top"><a href="javascript:void(0);" onclick="javascript:closewindow('<?php echo site_url("/user/question/cid/" . $qid); ?>');">[X]</a></td>
  </tr>
  </thead>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
    ...
    <td align="left"><input type="number" name="order" id="order" value="<?php echo $order;?>" step="1" style="width: 40px;" /><input name="qid" type="hidden" value="<?php echo $qid; ?>" /></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<table width="60%" border="0" style="float: right; border: 1px solid rgb(204, 204, 204); margin: 4% 19% 0px 0px; margin-bottom:40px;">
  <thead style='background-color: #ebedf0; color: #565e66; font-family: "Lucida Sans","Lucida Grande","Lucida Sans Unicode",Verdana,sans-serif;'>
  <tr>
    <td colspan="3" align="center"><h1>Question Listing</h1></td>
    </tr>
  </thead>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
    <td align="center" bgcolor="#CCCCCC" width="30%"><strong>Title</strong></td>
    <td align="center" bgcolor="#CCCCCC" width="30%"><strong>Order</strong></td>
    ...
    <tr>
      <td align="center" width="30%"><?php echo $v->title;?></td>
      <td align="center" width="30%"><?php echo $v->order;?></td>
      <td align="center" width="40%"><input type="button" class="btn btn-xs btn-success" name="answer" onclick="window.location.href = '<?php echo site_url("user/answer") . "/cid/" .$v->qqid; ?>';" id="answer" value="Answer" />
      <input type="button" class="btn btn-xs btn-info" name="update" onclick="window.location.href = '<?php echo site_url("user/question") . "/Edit/" . $v->qqid . "/" . $qid; ?>';" id="update" value="Update" />
      <input name="delete" type="button" class="btn btn-xs btn-danger" onclick="javascript:return confrm('<?php echo $qid;?>','<?php echo $v->qqid;?>');" value="Delete" /></td>
    </tr>
    <tr>
      <td height="10"></td>
      <td height="10"></td>
...
<script language="javascript">
	function confrm(url,id)
	{
		...
		{
			 window.location.href = 'http://localhost/can/user/question/delete/'+id+'/'+url;
			return true;
		}
		else
			return false;
