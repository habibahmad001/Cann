...
<body>
<form action="<?php if(isset($ID)) $ID = $ID; else $ID = ""; echo site_url("/user/" . $pageName . "/update/" . $ID . "/" . $course_id);?>" method="post" enctype="multipart/form-data" name="frm">
<div id="wrapper">
<input name="pagename" id="pagename" type="hidden" value="<?php echo site_url("/user/" . $pageName);?>" />
<input name="catid" id="catid" type="hidden" value="<?php echo $course_id;?>" />
<table width="60%" border="0" class="edit_course" style="float: right; border: 1px solid rgb(204, 204, 204); margin: 2% 19% 0px 0px; <?php if(!strpos($_SERVER['REQUEST_URI'], "/Edit/")) echo "display:none;";?>">
  ...
  <tr>
    <td>&nbsp;</td>
    <td colspan="2" align="center"><h1>Edit Section</h1></td>
    <td style="text-align:right;" align="right" valign="top"><a href="javascript:void(0);" onclick="javascript:closewindow('<?php echo site_url("/user/section/cid/" . $course_id); ?>');">[X]</a></td>
  </tr>
  </thead>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
    ...
    <td align="left"><input type="number" name="order" id="order" value="<?php echo $order;?>" step="1" style="width: 40px;" /><input name="course_id" type="hidden" value="<?php echo $course_id; ?>" /></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<table width="60%" border="0" style="float: right; border: 1px solid rgb(204, 204, 204); margin: 4% 19% 0px 0px; margin-bottom:40px;">
  <thead style='background-color: #ebedf0; color: #565e66; font-family: "Lucida Sans","Lucida Grande","Lucida Sans Unicode",Verdana,sans-serif;'>
  <tr>
    <td colspan="4" align="center"><h1>Section Listing</h1></td>
    </tr>
  </thead>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
    ...
    <td align="center" bgcolor="#CCCCCC" width="25%"><strong>Order</strong></td>
    <td align="center" bgcolor="#CCCCCC" width="25%"><strong>Description</strong></td>
    ...
    <tr>
      ...
      <td align="center" width="25%"><?php echo $v->order;?></td>
      <td align="center" width="25%"><?php echo $v->description;?></td>
      <td align="center" width="25%"><input type="button" class="btn btn-xs btn-success" name="section" onclick="window.location.href = '<?php echo site_url("user/content") . "/cid/" .$v->cscec_id; ?>';" id="content" value="Content" />
      <input type="button" class="btn btn-xs btn-info" name="update" onclick="window.location.href = '<?php echo site_url("user/section") . "/Edit/" . $v->cscec_id . "/" . $course_id; ?>';" id="update" value="Update" />
      <input name="delete" type="button" class="btn btn-xs btn-danger" onclick="javascript:return confrm('<?php echo $course_id;?>','<?php echo $v->cscec_id;?>');" value="Delete" /></td>
    </tr>
    <tr>
      <td height="10"></td>
      <td height="10"></td>
...
<script language="javascript">
	function confrm(url,id)
	{
		...
		{
			 window.location.href = 'http://localhost/can/user/section/delete/'+id+'/'+url;
			return true;
		}
		else
			return false;
